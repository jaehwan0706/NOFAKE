...
const verifyTokenMiddleware = (req, res, next) => {
    ...
    jwt.verify(token, getKey, { 
        ...
        req.user = {
            ...
            walletAddress: decoded.wallets?.[0]?.address,
            email: decoded.email
        };
        
        console.log("✅ 인증 성공 (카카오ID):", req.user.kakaoId);
        next();
    });
};
...
const participants = [];
const accountStorePath = path.join(__dirname, 'auth-accounts.json');
...
const loadAuthAccounts = () => {
    try {
        ...
        console.error('Failed to load auth account store:', error.message);
        return [];
    }
};

const saveAuthAccounts = (accounts) => {
    fs.writeFileSync(accountStorePath, JSON.stringify(accounts, null, 2));
};

const linkKakaoAccount = ({ kakaoId, walletAddress, email, joinedAt }) => {
    const normalizedWalletAddress = normalizeWalletAddress(walletAddress);

    if (!kakaoId) {
        return { ok: false, status: 400, error: 'KAKAO_ID_REQUIRED', message: '카카오 사용자 정보를 확인할 수 없습니다.' };
    }

    if (!normalizedWalletAddress) {
        return { ok: false, status: 400, error: 'WALLET_ADDRESS_REQUIRED', message: '지갑 주소를 확인할 수 없습니다.' };
    }

    const accounts = loadAuthAccounts();
    const existingByKakao = accounts.find((account) => account.kakaoId === kakaoId);
    const existingByWallet = accounts.find((account) => account.walletAddress === normalizedWalletAddress);

    if (existingByKakao && existingByKakao.walletAddress !== normalizedWalletAddress) {
        return {
            ok: false,
            status: 409,
            error: 'KAKAO_ALREADY_LINKED',
            message: '이미 이 카카오 계정으로 생성된 계정이 있습니다.',
            account: existingByKakao,
        };
    }

    if (existingByWallet && existingByWallet.kakaoId !== kakaoId) {
        return {
            ok: false,
            status: 409,
            error: 'WALLET_ALREADY_LINKED',
            message: '이미 다른 카카오 계정에 연결된 지갑입니다.',
            account: existingByWallet,
        };
    }

    if (existingByKakao) {
        existingByKakao.email = email || existingByKakao.email || '';
        existingByKakao.lastLoginAt = new Date().toISOString();
        saveAuthAccounts(accounts);
        return { ok: true, account: existingByKakao, isNew: false };
    }
...
app.post('/api/login', verifyTokenMiddleware, (req, res) => {
    const walletAddress = req.body.address || req.body.walletAddress || req.user.walletAddress;
    const joinedAt = req.body.joinedAt || new Date().toISOString();
    const accountLink = linkKakaoAccount({
    ...
    if (!accountLink.ok) {
        return res.status(accountLink.status).json({
            success: false,
    ...
    const participant = upsertParticipant({
        name: req.body.name || req.user.email || `참여자 ${normalizedWalletAddress?.slice(-4) || ''}`,
        email: req.body.email || req.user.email || '',
        walletAddress: normalizedWalletAddress,
        kakaoId: req.user.kakaoId,
        joinedAt,
    ...
    res.json({
        success: true,
        user: { ...req.user, walletAddress: normalizedWalletAddress },
        participant,
